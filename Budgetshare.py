no_of_pax = int(input("Enter no of people: "))
...
try:
    name = input(f"Enter {no_of_pax} names, seperated by commas: ")
    namelist = [x.strip() for x in name.split(",")]
    moolah = input(f"Enter {no_of_pax} amounts in the same order as above, seperated by commas: ")
    paid_py_person = [int(x.strip()) for x in moolah.split(",")]
    


except ValueError:
    while isinstance(moolah, int) == False:
        moolah = input(f"Enter amount paid by {name}: ")
        moolah = int(moolah)
